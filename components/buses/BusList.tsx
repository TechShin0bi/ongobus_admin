...
export const BusList = ({ buses, selectedBusId, onSelectBus, agencies }: BusListProps) => {
  const t = useTranslations('Buses');
  const [isModalOpen, setIsModalOpen] = useState(false);

  const getStatusTranslationKey = (status: Bus['status']) => {
    const key = `status.${status.toLowerCase()}` as 'status.active' | 'status.maintenance' | 'status.retired';
    return key;
  };
  ...
  return (
    <>
      <div className="xl:col-span-4 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden h-[calc(100vh-140px)] flex flex-col">
        <div className="p-4 border-b border-gray-100 bg-white sticky top-0 z-10">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-1.5 bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors">
          ...
          {buses.map((bus) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-1">{t(`types.${bus.type.toLowerCase().replace('-', '')}`)} • {bus.seats} {t('seats')}</p>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={t('addNewBus')}>
        <AddBusForm agencies={agencies} onClose={() => setIsModalOpen(false)} />
      </Modal>
    </>
  );
};